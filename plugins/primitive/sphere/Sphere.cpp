...
std::string Sphere::getAuthor() const
{
    return "Jiachen";
}
...
{
    return "Sphere Plugin";
}

Engine::Version Sphere::getVersion() const
{
    return {1, 0, 0};
}

Engine::Entity::EntityType Sphere::getEntityType() const
{
    return Engine::Entity::EntityType::PRIMITIVE;
}

Engine::System::SystemType Sphere::getSystemType() const
{
    return Engine::System::SystemType::UNKNOWN;
}

ECS::Signature Sphere::getEntitySignature() const
{
    return _signature;
}

bool Sphere::initialize(std::shared_ptr<Engine::Scene> scene)
{
...
bool Sphere::shutdown()
{
    return true;
}

std::vector<Engine::EntityFactoryInfo> Sphere::getEntityFactories()
{
    return {
    {
            getName(),
...
std::vector<Engine::SystemFactoryInfo> Sphere::getSystemFactories()
{
    return {};
}

void Sphere::configureSystems(std::shared_ptr<Engine::Scene> scene)
{
    (void)scene;
}

void Sphere::configureEntityInfo(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene)
{
    Utils::Logger::getInstance().debug("Sphere: Adding EntityInfo component");
    entity->addComponent<Engine::CoreComponents::EntityInfo>("sphere", Engine::Entity::EntityType::PRIMITIVE);
    scene.getEntityManager()->onComponentAdded<Engine::CoreComponents::EntityInfo>(entity->getID());
}

void Sphere::configurePosition(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Position");
...
void Sphere::configureRotation(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Rotation");
...
void Sphere::configureScale(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Scale");
...
void Sphere::configureColor(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Color");
...
void Sphere::configureMaterial(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Material");
...
void Sphere::configureRadius(std::shared_ptr<ECS::Entity> entity, Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Init Radius");
...
std::shared_ptr<ECS::Entity> Sphere::createSphereEntity(Engine::Scene &scene, 
    const Engine::Entity::EntityConfig& config)
{
    Utils::Logger::getInstance().debug("Sphere: Creating sphere entity");
    auto entity = scene.getEntityManager()->createEntity();

    configureEntityInfo(entity, scene);
    configurePosition(entity, scene, config);
    configureRotation(entity, scene, config);
    configureScale(entity, scene, config);
    configureColor(entity, scene, config);
    configureMaterial(entity, scene, config);
    configureRadius(entity, scene, config);

    return entity;
}

EXPORT_PLUGIN(Sphere)
